...
When(/^I Hover over the profile image of user2$/, async () => {
  await HoverPage.locateUserImage(imageNumber).moveTo();
});

Then(
  /^I see name: user 2 and view profile link are displayed under the image on hover$/,
  async () => {
    await expect(HoverPage.locateUserProfile(imageNumber)).toBeDisplayed();
  }
);
...
When(/^I Enter any text into the prompt and click ok$/, async () => {
  this.sendText = Randomizing.generateRandomText(5);
